...
beforeAll( async () => {
  ...
  const users = await app.db('users').insert([
    { name: 'User #1', mail: 'user@mail.com', password: '$2a$10$woCk439YSlf9NY4gdbxEjO7Tyuv7uifi2EKLpGFTdHK0gHM8VE.12'},
    { name: 'User #2', mail: 'user2@mail.com', password: '$2a$10$woCk439YSlf9NY4gdbxEjO7Tyuv7uifi2EKLpGFTdHK0gHM8VE.12'},
  ], '*');
  [user, user2 ] = users;
...
test('Deve listar apenas as transações do usuário', () => {
  return app.db('transactions').insert([
    { description: 'T1', date: new Date(), ammount: 1000, type: 'I', acc_id: accUser.id },
    { description: 'T2', date: new Date(), ammount: 1500, type: 'O', acc_id: accUser2.id }
...
// }); // forma válida para validar o teste sem q acc_id ainda não tenha sido criado
  ...
  test('Não deve inserir uma transação com tipo inválido', () => testTemplate({ type: 'X' }, 'Tipo inválido'));
...
test('Deve remover uma transação', () => {
  return app.db('transactions').insert(
    { description: 'to Remove', date: new Date(), ammount: 100, type: 'I', acc_id: accUser.id }, ['id']
...
test('Não deve remover a transação de outro usuário ', () => {
    return app.db('transactions').insert(
